...
(function(){
  ...
  function injectCSS(){
    ...
    .tril-card{background:#fff;color:#1f2937;width:340px;max-width:92vw;padding:24px;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.4)}
    ...
    #trilAdminPanel{position:fixed;top:48px;right:10px;width:380px;max-width:94vw;max-height:84vh;overflow:auto;z-index:99991;
    ...
    #trilAdminPanel .closex{position:absolute;top:8px;right:10px;cursor:pointer;color:#93a0bd;font-size:16px}
  ...
  function doLogout(){
    ...
    renderTopbar(); applyGating(); var p=el('trilAdminPanel'); if(p) p.classList.remove('show');
  }
  ...
  function modulesUI(){
    ...
    var html='<div class="closex" onclick="TrilAuth.toggleModules(false)">✕</div><h4>扩展模块(管理员)</h4>';
  ...
  function renderAdmin(){
    ...
    p.innerHTML='<span class="closex" onclick="TrilAuth.toggleAdmin(false)">✕</span><h4>📊 登录记录 / 管理（管理员）</h4>'+
      '<div class="kpi" id="adKpi"></div>'+
      '<div id="adRegion"></div>'+
      '<h4 style="margin-top:10px">近期登录</h4><div id="adRecent" style="max-height:160px;overflow:auto"></div>'+
      '<h4 style="margin-top:10px">用户管理</h4>'+
      '<div id="adUsers"></div>'+
  ...
  function renderTopbar(){
    ...
    if(state.online && state.token){
      bar.innerHTML='<span class="u">👤 '+esc(state.user)+'（'+(state.role==='admin'?'管理员':'普通')+'）</span>'+
        ...
        '<button onclick="TrilAuth.logout()">退出</button>';
      var p=el('trilAdminPanel'); if(state.role==='admin'&&p){ renderAdmin(); }
    } else if(state.online){
      bar.innerHTML='<span class="u">未登录</span><button onclick="TrilAuth.showLogin()">登录</button>';
    } else {
      bar.innerHTML='<span class="u">📴 离线模式（无后台，仅本地）</span>';
    }
  }
  ...
  window.TrilAuth = {
    init: function(){
      ...
      // 离线检测
      if(state.online){
        fetch((API||'')+'/api/health',{method:'GET'}).then(function(){ return true; }).catch(function(){ state.online=false; });
      }
      // 已有 token?
